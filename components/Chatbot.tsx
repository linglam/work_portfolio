    ...
    return (
        <>
            <button
                ...
                className={`fixed bottom-6 right-6 bg-sky-500 text-white p-4 rounded-full shadow-lg hover:bg-sky-600 transition-transform transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-zinc-950 ${isOpen ? 'scale-0' : 'scale-100'}`}
            ...
            <div
                className={`fixed bottom-6 right-6 sm:bottom-8 sm:right-8 w-[calc(100%-3rem)] sm:w-full max-w-md h-[75%] sm:h-auto sm:max-h-[80vh] bg-white/80 dark:bg-zinc-900/80 backdrop-blur-lg rounded-xl shadow-2xl flex flex-col transition-all duration-300 ease-in-out origin-bottom-right ${isOpen ? 'scale-100 opacity-100' : 'scale-95 opacity-0 pointer-events-none'}`}
                role="dialog"
                aria-modal="true"
                aria-labelledby="chat-heading"
            >
                <header className="flex items-center justify-between p-4 border-b border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-950/50 rounded-t-xl">
                    <h2 id="chat-heading" className="text-lg font-bold text-zinc-900 dark:text-zinc-100">AI Assistant</h2>
                    <button onClick={() => setIsOpen(false)} className="text-zinc-500 dark:text-zinc-400 hover:text-black dark:hover:text-white" aria-label="Close chat">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`flex gap-3 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            {msg.role === 'model' && <BotIcon className="w-6 h-6 text-sky-500 dark:text-sky-400 flex-shrink-0 mt-1" />}
                            <div
                                className={`max-w-xs md:max-w-sm px-4 py-2 rounded-xl prose prose-sm dark:prose-invert ${msg.role === 'user' ? 'bg-sky-600 text-white rounded-br-none' : 'bg-zinc-100 text-zinc-800 dark:bg-zinc-800 dark:text-zinc-300 rounded-bl-none'}`}
                    ...
                    {isLoading && (
                        <div className="flex justify-start gap-3">
                            <BotIcon className="w-6 h-6 text-sky-500 dark:text-sky-400 flex-shrink-0 mt-1" />
                            <div className="flex items-center space-x-1 px-4 py-3 bg-zinc-100 dark:bg-zinc-800 rounded-xl rounded-bl-none">
                                <span className="w-2 h-2 bg-sky-500 dark:bg-sky-400 rounded-full animate-pulse" style={{ animationDelay: '0s' }}></span>
                                <span className="w-2 h-2 bg-sky-500 dark:bg-sky-400 rounded-full animate-pulse" style={{ animationDelay: '0.2s' }}></span>
                                <span className="w-2 h-2 bg-sky-500 dark:bg-sky-400 rounded-full animate-pulse" style={{ animationDelay: '0.4s' }}></span>
                            </div>
                        </div>
                    )}
                ...
                <form onSubmit={handleSendMessage} className="p-4 border-t border-zinc-200 dark:border-zinc-800">
                    <div className="flex items-center space-x-2">
                        <input
                            ...
                            className="flex-1 w-full bg-white dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 rounded-lg px-4 py-2 text-zinc-800 dark:text-zinc-200 placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-sky-500"
                            disabled={isLoading}
                            aria-label="Chat input"
                        />
                        <button
                            type="submit"
                            className="bg-sky-500 text-white p-3 rounded-lg hover:bg-sky-600 disabled:bg-zinc-400 dark:disabled:bg-zinc-700 disabled:cursor-not-allowed"
